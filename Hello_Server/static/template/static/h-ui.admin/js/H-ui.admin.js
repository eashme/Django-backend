...
/*自定义封装发送ajax请求,超时添加loading层*/
function getAjax(method,api_url,datas,callback) {
	var layerIndex;
	...
	var xhr = $.ajax({
		...
		success:function (json) {
			layer.close(layerIndex);    // 成功后，隐藏loading
		...
		error: function (textStatus) {
		  ...
		  complete: function (XMLHttpRequest,status) {
				if(status == 'timeout') {
					xhr.abort();    // 超时后中断请求
					layer.msg(urequest.message, {icon: 2, time: 1000}, function () {
						location.reload();
					})
				}
			}
